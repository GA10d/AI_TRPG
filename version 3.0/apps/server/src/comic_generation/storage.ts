...
async function rebuildManifestFromFiles(comicRoot: string): Promise<ComicProjectSummary[]> {
  await ensureComicRoot(comicRoot);
  const entries = await readdir(comicRoot, {
    withFileTypes: true
  });
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Skip malformed projects and keep the rest usable.
    }
  }
...
export async function deleteComicProjectFromDisk(
  comicRoot: string,
  comicId: string
): Promise<boolean> {
