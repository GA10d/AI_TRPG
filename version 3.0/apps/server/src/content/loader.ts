...
export async function loadContentCatalog(contentRoot: string): Promise<ContentCatalogEntry[]> {
  ...
  for (const ruleDirectory of ruleDirectories) {
    ...
    let rulePackage: LoadedRulePackage;

    try {
      rulePackage = await loadRulePackage(normalizedRoot, ruleId, DEFAULT_LOCALE);
    } catch (error) {
      console.warn(`[content-catalog] skip rule ${ruleId}: ${error instanceof Error ? error.message : String(error)}`);
      continue;
    }
    ...
    const stories: ContentCatalogEntry["stories"] = [];
    for (const storyItem of storyItems) {
      ...
      try {
        ...
        console.warn(
          `[content-catalog] skip story ${ruleId}/${storyItem.name}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }

    if (stories.length === 0) {
      console.warn(`[content-catalog] skip rule ${ruleId}: no valid stories`);
      continue;
    }
