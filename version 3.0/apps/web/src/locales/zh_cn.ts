export const zhCn = {
  ...
  app: {
    ...
    status: {
      ...
      prepareRoundLogStart: (round: number) => `开始准备第 ${round} 轮队伍草稿。`,
      prepareRoundLogPrimaryAi: (name: string) => `阶段 1：正在生成 AI 主角「${name}」的草稿。`,
      prepareRoundLogPrimaryHuman: "阶段 1：已记录主玩家输入。",
      prepareRoundLogCompanions: (count: number, names: string) =>
        `阶段 2：将并行生成 ${count} 名 AI 队友草稿${names ? `：${names}` : ""}。`,
      prepareRoundLogWait:
        "正在等待服务端完成草稿生成；如果长时间停在这里，通常说明模型仍在生成草稿。",
      prepareRoundLogDone: (count: number) => `草稿准备完成，共返回 ${count} 条本轮草稿。`,
      prepareRoundLogFailed: (reason: string) => `本轮草稿准备失败：${reason}`,
      ...
      commitRoundLogCountdown: (seconds: number) => `草稿已就绪，${seconds} 秒后自动统一提交。`,
      commitRoundLogStart: (round: number, count: number) =>
        `开始提交第 ${round} 轮，共 ${count} 条队伍输入。`,
      ...
      dismissEndingUnavailable: "当前没有可撤销的结局状态。",
      dismissEndingPending: "正在撤销这次结局误报...",
      dismissEndingSuccess: "结局状态已撤销，现在可以继续正常游戏。",
  ...
  gameScreen: {
    ...
    reasoningTitle: "模型推理记录",
    reasoningCount: (count: number) => `${count} 条`,
    ...
    reasoningEmpty: "当前还没有可查看的推理内容。",
    reasoningOutputLabel: "Output",
    reasoningContentLabel: "Reasoning content",
    reasoningCommittedLabel: "已提交",
    reasoningPrimaryDraftLabel: "主玩家草稿",
    reasoningCompanionDraftLabel: "AI队友草稿",
    ...
    activityLogEyebrow: "运行日志",
    activityLogTitle: "自动流程与状态",
    activityLogCount: (count: number) => `${count} 条`,
    activityLogCurrentStatus: "当前状态",
    activityLogIdle: "目前没有新的运行日志。",
    activityLogEmpty: "当前还没有可显示的运行日志。",
    activityLogErrorLabel: "错误",
    ...
    dismissEndingFalsePositive: "结局误报",
    dismissEndingFalsePositiveSubmitting: "撤销中...",
    ...
    comicEyebrow: "漫画",
    comicTitle: "世界线漫画",
    comicDescription: "这里会显示这条世界线已经生成的漫画页。",
    comicCount: (count: number) => `${count} 张漫画`,
    comicEmpty: "这条世界线暂时还没有生成漫画。",
    comicGeneratingHint: "漫画正在后台生成，稍后会自动出现在这里。",
    comicPageLabel: (index: number) => `漫画 ${index}`,
    comicPageAlt: (index: number) => `漫画 ${index}`,
    comicOpenLightbox: "显示大图",
    comicPrevPage: "上一张",
    comicNextPage: "下一张",
    comicGenerationStart: (index: number) => `正在生成漫画 ${index}...`,
    comicGenerationDone: (index: number) => `漫画 ${index} 已生成。`,
    comicAlreadyExists: (index: number) => `漫画 ${index} 已存在，直接复用。`,
    comicGenerationFailed: (message: string) => `漫画生成失败：${message}`,
    comicLoadFailed: (message: string) => `漫画读取失败：${message}`,
    ...
    memoryDebugDescription:
      "这里展示当前 session memory、Narrator context pack，以及队友公开回合会读到的上下文包。",
    memoryRebuild: "重建 Memory",
    memoryRebuilding: "重建中...",
    memoryFacts: (count: number) => `事实：${count} 条`,
    memoryOpenLoops: (count: number) => `未解问题：${count} 条`,
    memoryEpisodes: (count: number) => `场景摘要：${count} 条`,
    noMemoryDebugData: "当前还没有可展示的 memory / context pack 调试数据。",
    narratorContextPack: "Narrator Context Pack",
    companionContextPack: (name: string) => `${name} 的公开回合 Context Pack`,
    replayLog: "回放日志",
  ...
  gameSetupScreen: {
    ...
    fields: {
      ...
      comicStyleLabel: "漫画画风",
      comicStyleHint: "决定新建世界线漫画项目的默认画风；同一世界线生成第一页后，后续会沿用已建立的风格。",
      comicStyleLoadingHint: "正在读取可用的漫画画风预设。",
      comicStyleUnavailableHint: "暂时没能读取漫画画风预设；如果直接开局，后端会回退到默认风格。",
      comicStyleLoadingOption: "正在加载画风预设...",
      comicStyleUnavailableOption: "画风预设暂不可用",
      logViewLabel: "日志显示",
    ...
    model: {
      ...
      comicStyle: (value: string) => `漫画画风：${value}`,
    ...
    advancedModel: {
      ...
      companionTitle: (value: string) => `AI 队友：${value}`,
      companionDescription: "用于这名 AI 队友的公开回合草稿与私聊回复。",
      companionFallback: (index: number) => `AI队友 ${index}`,
      noCompanions: "当前还没有 AI 队友；添加队友后就能分别给他们指定模型。"
    },
    preview: {
      eyebrow: "开场预览",
      title: "开场预览",
    ...
    companions: {
      eyebrow: "同行者",
      title: "同行者设置",
      description: "这里集中管理 AI 队友名单、名字、人格标签和外形设定，并保留后续私聊扩展位。",
      entryTitle: "AI 同伴入口",
      ...
      emptyDescription: "点击下方按钮先添加一名 AI 队友，然后为它设置名字、性格和形象。",
      addTitle: "添加同伴",
      addDescription: "最多支持 3 名 AI 队友。每名队友都可以自定义名字，并多选性格与形象标签。",
      addButton: "+ 添加同伴",
      ...
      configureAppearanceButton: "设置形象",
